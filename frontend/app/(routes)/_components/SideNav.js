...
function SideNav({toggleSideBar}) {
  ...
  return (
    <div className=' h-full p-5 border '>
        ...
        {!user?<Link href='/sign-in'><Button variant="ghost" className='flex gap-2 items-center cursor-pointer'><LogIn/>
          Sign In</Button></Link>
        : <div className='flex items-center gap-3'><UserButton/>Profile</div>}
        </div>
    </div>
  )
}
